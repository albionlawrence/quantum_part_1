...
A \to U A U^{\dagger} = A'
```
In the case that the transformation is infinitesimal, $U = {\bf 1} - i \eps K + \cO(\eps^2)$ with $K$ Hermitian, then to $\cO(\eps)$, we have
```{math}
A' = A - i \eps [K,A]
```
We can compare this to the change in classical observables under a transformation generated by an observable $K(p,q)$: for this $\delta A = - \{K, A\}$ where $\{ , \}$ is the Poisson bracket.
...
Let us get our heads around the concept of the transformation of operators by startig with the position operator; this, because it has vector indices, so we might sensibly thin k it transforms like a vector, which it does! To see this, consider the matrix element of the transformed operator:
```{math}
\begin{align}
...
Let us consider a spherical tensor operator $\cO^{j_1}_{m_1}$ acting on a state $\ket{\alpha, j_2 m_2}$. where $\alpha$ denote sadditional quantum numbers. Under finite rotations, this transforms as
```{math}
\begin{align}
